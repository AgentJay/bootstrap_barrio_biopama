...
function getNodeRestResults(activeIndicator) {
	...
	jQuery.ajax({
		...
		success: function(d) {
			...
			if (d.hasOwnProperty("records")) { //from a JRC REST Service
				...
				if (d.metadata.recordCount == 0) {
					...
					getNodeChart(activeIndicator, indicator, 2);
				}
				else {
					//create the map
		...
		error: function() {
			...
			getNodeChart(activeIndicator, indicator, 1);
		}
	});
}
...
function getNodeChart(key, indicator, error) {
